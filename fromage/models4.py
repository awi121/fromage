...
class FrozenArgs:
  freeze_lm: bool = True
  freeze_vm: bool = True
  opt_version: str = 'facebook/opt-1.3b'
...
class FromageModel(nn.Module):
  ...
  def get_visual_embs(self, feature, mode: str = 'qa',type1='img'):
    if mode not in ['captioning', 'retrieval','qa']:
  ...
  def forward(
    ...
    visual_embs = self.get_visual_embs(features, mode,type1)
    ...
    if input_prefix is not None:
      ...
      # assert input_prefix.shape[0] == batch_size
      # assert input_prefix.shape[2] == input_embs.shape[2]
    ...
    #     truncation=True,
    #     max_length=32)
        prompt_ids = self.tokenizer(input_prefix, add_special_tokens=False, return_tensors="pt",padding='max_length',truncation=True,max_length=32).input_ids
        prompt_ids = prompt_ids.to(visual_embs.device)
        prompt_embs = self.input_embeddings(prompt_ids)
  ...
  def generate(self, embeddings = torch.FloatTensor, max_len: int = 32,
               ...
               ret_scale_factor: float = 0.0, filter_value: float = -float('Inf')):
...
class Fromage(nn.Module):
  ...
  def __call__(self, features, tgt_tokens: Optional[Tensor] = None, caption_len: Optional[Tensor] = None,
               ...
               input_prefix: Optional[str] = None, inference: bool = False,type1='img') -> Tensor:
    if generate:
      return self.model.generate(features, num_words, temperature=temperature, top_p=top_p,
      ...
      output = self.model(
        ...
        inference = inference,type1=type1)
      return output
  ...
  def generate_for_images_and_texts(
    ...
    for i, p in enumerate(prompts):
      if type(p) == list:
        ...
        visual_embs = self.model.get_visual_embs(p, mode='qa')  # (1, n_visual_tokens, D)
...
def load_fromage(model_dir: str,type1= 'pretrained') -> Fromage:
  ...
  # Example code to load model with key mapping

  state_dict = torch.load(model_ckpt_path)
  new_state_dict = {}
  for key, value in state_dict['state_dict'].items():
      new_key = key.replace('module.model.', 'model.')  # adjust the replacement as needed
